            ...
            for link in cat.get_links():
                if skip_unresolved and not link.is_resolved():
                    ...
                    if isinstance(link.target, pystac.Item):
                        setter_funcs.append(process_item(link.target, new_root))
                elif link.rel == pystac.RelType.CHILD:
                    link.resolve_stac_object(root=self.get_root())
                    if isinstance(link.target, pystac.Catalog) or isinstance(
                        link.target, pystac.Collection
                    ):
                        setter_funcs.extend(
                            process_catalog(link.target, new_root, is_root=False)
                        )
